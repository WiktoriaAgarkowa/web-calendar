...
api = Api(app)  # Create REST endpoints

# Create an RequestParser object for parsing POST requests:
...
# Specify arguments (name, type and error message):
parser.add_argument(
    'date',
    type=inputs.date,
...
class TodayEventsResource(Resource):
    # Method gets today events from API
...
class EventByID(Resource):
    # Method gets event from database by ID
    @marshal_with(resource_fields)
    def get(self, id):
    ...
    def delete(self, id):
        # Method delete event from database by ID
        event = Event.query.filter(Event.id == id).first()
        if event is None:
            abort(404, "The event doesn't exist!")
...
class EventResource(Resource):
    def post(self):
        # Method for adding event to database and API response
    ...
    @marshal_with(resource_fields)
    def get(self):
        # Method for getting events from a certain period
